...
class DownloadsActivity : AppCompatActivity(), AdapterView.OnItemClickListener, ActiveDownloadsModel.Listener, AdapterView.OnItemLongClickListener{
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        vb.ibCloseDownloads.setOnClickListener { finish() }
        ...
        downloadsHistoryModel.lastLoadedItems.subscribe(this, false, {
            if (it.isNotEmpty()) {
                vb.llEmptyState.visibility = View.GONE
                adapter.addItems(it)
                vb.listView.requestFocus()
            } else if (adapter.count == 0) {
                vb.llEmptyState.visibility = View.VISIBLE
            }
        })
    ...
    private fun updateStorageSubtitle() {
        try {
            ...
            val activeCount = activeDownloadsModel.activeDownloads.size
            if (activeCount > 0) {
                vb.tvDownloadsSubtitle.text = "$activeCount active · $freeStr free"
            } else {
                vb.tvDownloadsSubtitle.text = "Nothing in progress · $freeStr free"
            }
        } catch (e: Exception) {
            vb.tvDownloadsSubtitle.text = "Downloads & Files"
        }
    }

    private fun setupFilterPills() {
        val pills = listOf(vb.btnFilterAll, vb.btnFilterActive, vb.btnFilterCompleted, vb.btnFilterFailed)
        pills.forEach { button ->
            button.setOnClickListener {
                pills.forEach { p ->
                    p.isSelected = false
                    p.setTextColor(Color.parseColor("#94A3B8"))
                }
                button.isSelected = true
                button.setTextColor(Color.WHITE)
            }
        }
        vb.btnFilterAll.isSelected = true
        vb.btnFilterAll.setTextColor(Color.WHITE)
    }
