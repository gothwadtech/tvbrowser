    ...
    fun lastSSLError(getDetails: Boolean): String {
        ...
        return if (getDetails) {
            ...
            when (lastSSLError.primaryError) {
                SslError.SSL_EXPIRED -> BrowserApp.instance.getString(R.string.ssl_expired)
                SslError.SSL_IDMISMATCH -> BrowserApp.instance.getString(R.string.ssl_idmismatch)
                SslError.SSL_DATE_INVALID -> BrowserApp.instance.getString(R.string.ssl_date_invalid)
                SslError.SSL_INVALID -> BrowserApp.instance.getString(R.string.ssl_invalid)
                else -> "unknown"
            }
        }
    }
